...
parser.add_argument('-t', '--type', dest='scan_type', type=str, help='This is to establish what type of scanning to use. Options are: public_ip, private_ip, kube, manual', default='manual')
parser.add_argument('-i', '--ip', dest='manual_ips', type=str, help='If running a scan type of manual, provide the target address(es) here.')
...
parser.add_argument('-c', '--cloud-provider', dest='provider', type=str, help='The cloud provider where the tool is being used, if using provider-specific options. This will default to manual for a manual scan unless otherwise specified. Options are: aws', default='manual')
